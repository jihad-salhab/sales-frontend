...
import { LanguagesMeta } from '../i18next';

class Localization {
  isRTL = (): boolean => {
    for (let i = 0; i < LanguagesMeta.length; i++)
      if (LanguagesMeta[i].name === i18n.language) return LanguagesMeta[i].isRTL;
    return false;
  };
  ...
  getFloat = (reversed: boolean = false) => {
    if (this.isRTL()) return reversed ? 'right' : 'left';
    else return reversed ? 'left' : 'right';
  };
